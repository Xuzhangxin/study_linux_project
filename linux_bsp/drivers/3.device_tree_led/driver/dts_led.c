...
static ssize_t led_write(struct file *file, const char __user *data, size_t cnt, loff_t *off)
{
    ...
    printk("user led driver write sucess, buf[0]:%d", private_dev->m_buf[0]);
    return ret;
}
...
static int __init led_init(void)
{
    ...
    /// 获取设备树节点
    led_dev.nd = of_find_node_by_path("/dtsled");
    if (!led_dev.nd)
    {
        printk("xzxled node not found\n");
        return -EINVAL;
    }
    ...
    if (ret < 0)
    {
        printk("xzxled not found\n");
        return -EINVAL;
    }

    printk("regdata begin\n");
    int i = 0;
    for (; i < sizeof(regdata) / sizeof(regdata[0]); i++)
    {
        printk("%#X ", regdata[i]);
    }
    printk("regdata end\n");
    ...
    GPIO1_GDIR = ioremap(regdata[8], regdata[6]);
